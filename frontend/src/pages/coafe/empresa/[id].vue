<template>
    <div>
      <h5 class="mb-0 p-2 mt-4">Contratante <strong>{{ contratante?.nome }}</strong></h5>
  
      <div class="col-12">
        <div class="card flex-column p-fluid col-12">
          <div class="formgrid grid flex flex-column justify-content-between gap-10">
            <div class="field col">
              <label for="nome">Nome</label>
              <InputText :disabled="!edicao.ativa" id="nome" type="text" v-model="state.nome" />
            </div>
  
            <div class="field col">
              <label for="tipo" class="flex flex-direction col">Tipo de Contratante</label>
              <SelectButton
            ...
            <div v-if="state.cpf" class="field col">
              ...
              <InputText :disabled="!edicao.ativa" id="cpf" type="text" v-model="state.cpf" />
            </div>
            <div v-else="state.cnpj" class="field col">
              <label for="cnpj">CNPJ</label>
              <InputText :disabled="!edicao.ativa" id="cnpj" type="text" v-model="state.cnpj" />
            </div>
            <div class="field col">
      ...
      const handleUpdateContratante = async () => {
        ...
        try {
          ...
          edicao.ativa = false; // Desabilita a edição após a atualização
            ...
            toast.add({
            severity: "error",
            summary: "Erro ao atualizar contratante: " + e,
            life: 3000,
          });
          console.log(e);
        }
