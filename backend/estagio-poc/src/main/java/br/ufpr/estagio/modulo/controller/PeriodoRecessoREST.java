	...
	public ResponseEntity<PeriodoRecessoDTO> inserir(@RequestBody PeriodoRecessoDTO periodoRecesso) {
		try {
			PeriodoRecesso newPeriodoRecesso = periodoRecessoService
					.novo(mapper.map(periodoRecesso, PeriodoRecesso.class));
		...
		try {
			...
			if (lista.isEmpty()) {
				...
				return ResponseEntity.status(HttpStatus.OK).body(
						lista.stream().map(e -> mapper.map(e, PeriodoRecessoDTO.class)).collect(Collectors.toList()));
			}
	...
	public ResponseEntity<Object> atualizar(@PathVariable String id, @RequestBody PeriodoRecessoDTO periodoRecesso) {
		try {
			...
			if (periodoRecessoOptional.isEmpty()) {
				...
				long diferencaMilissegundos = periodoRecesso.getDataFim().getTime()
						- periodoRecesso.getDataInicio().getTime();
				...
				PeriodoRecesso periodoRecessoAtualizado = periodoRecessoService
						.atualizarDados(periodoRecessoOptional.get(), periodoRecesso);
				return ResponseEntity.status(HttpStatus.OK)
						.body(mapper.map(periodoRecessoAtualizado, PeriodoRecessoDTO.class));
			}
		} catch (NotFoundException ex) {
			ex.printStackTrace();
			ErrorResponse response = new ErrorResponse(ex.getMessage());
